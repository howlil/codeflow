...
Repository foundation design is defined. No product code has been scaffolded yet.
...
|---|---|---|
| M0 | executable monorepo + CI quality gates | NEXT |
| M1 | TypeScript full-stack fixture -> semantic flow -> interactive canvas -> evidence inspector | PLANNED |
...
pnpm workspace
  ...
  + packages/contracts only if needed
  + strict TypeScript
  + test/typecheck/lint/build gates
  + minimal smoke UI/API
```

M0 must not introduce product complexity that belongs to later milestones.
