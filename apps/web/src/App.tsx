...
export function App() {
  ...
  async function analyzeRepository(
    ...
    try {
      ...
      setFlow(null);
      setError(errorMessage(caughtError, 'Unable to analyze the selected repository.'));
    } finally {
      setAnalyzing(false);
    }
  }
  ...
  async function analyzeGitHub(
    ...
    try {
      ...
      setFlow(null);
      setError(
        errorMessage(caughtError, 'Unable to analyze the public GitHub repository.'),
      );
    } finally {
      setAnalyzing(false);
    }
  ...
  async function analyzePullRequest(pullRequestUrl: string) {
    ...
    try {
      ...
      setChangeAnalysis(null);
      setError(
        errorMessage(caughtError, 'Unable to visualize the public pull request.'),
      );
    } finally {
      setAnalyzing(false);
    }
    ...
    <main className="workspace-shell graph-app-shell">
      <header className="workspace-header graph-app-header">
        <div className="workspace-brand">
          <h1>CodeFlow</h1>
          <span>See where code starts, where it goes, and what it depends on.</span>
        </div>
        <IconButton
      ...
      {analyzing ? (
        <section className="graph-loading-state" role="status">
          ...
          <span>
            Finding entry points and source-backed relationships without executing repository code.
          </span>
        </section>
      ) : flow !== null ? (
        <GraphWorkspace
...
function AcquisitionWorkspace({
  ...
  return (
    <section className="graph-acquisition" aria-labelledby="graph-acquisition-title">
      <div className="graph-acquisition-intro">
        ...
        <p>
          Open a repository, start from an entry point or symbol, then follow calls,
          references, dependencies, and type relationships through one semantic graph.
        </p>
      </div>
      ...
      {error !== null ? (
        <div className="graph-acquisition-error" role="alert">
          <span>{error}</span>
          <Button variant="ghost" onClick={onClearError}>Dismiss</Button>
        </div>
      ) : null}
      ...
      <div className="graph-acquisition-secondary">
        ...
        <details>
          ...
          <GitHubPullRequestPicker busy={false} onAnalyze={onAnalyzePullRequest} />
        </details>
      </div>
    </section>
  );
